		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(7); p.SoundPath = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(8); p.isOnOption = (cell == null ? false : cell.BooleanCellValue);
					cell = row.GetCell(9); p.selection1 = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(10); p.selection2 = (cell == null ? "" : cell.StringCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
